...
> **Note**
> The iOS part of Compose Multiplatform is in Alpha. It may change incompatibly and require manual
> migration in the
> future.
...
own [Compose Multiplatform](https://github.com/JetBrains/compose-multiplatform#readme) application
targeting desktop,
Android, and iOS.
...
You can now make some changes in the code and check that they are visible in both the iOS and
Android applications at
the same time:
...
1. In Android Studio, navigate to the `shared/src/commonMain/kotlin/App.kt` file.
   ...
   Here, you see the code responsible for rendering the "Hello, World!" button and the animated
...
3. Re-run the `desktopApp`, `androidApp`, and `iosApp` configurations. You'll see this change
